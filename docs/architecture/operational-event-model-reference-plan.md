...
| Projection record ABI | Canonical record shape | Helper code, shared materialized registry, status-card bridge, diagnostics correlation, and `data/projectionRecords` Yjs materialization added |
...
| Shared dispatcher | Per-webspace demanded refresh | Base dispatcher/API, status-card wildcard handler, canonical record materialization, Yjs projection-record cache write, and Infrascope-specific demanded refresh handler added |
