...
- `/api/node/status-cards/infrascope/demanded-only-contract` exposes the
  demanded-only per-webspace refresh contract, including
  `demanded_projection_keys(webspace_id)`, `demanded_only=true`, skipped
  no-demand behavior, and the no-cross-webspace-churn boundary
- `/api/node/projection-migration/acceptance-summary` includes
